...
class Task632 {
    ...
    public int[] smallestRange(List<List<Integer>> nums) {
        ...
        var slideWindow = new TreeSet<Triple>(Comparator.comparingInt((Triple triple) -> triple.value).thenComparingInt(triple -> triple.rowIndex));
        ...
        while (true) {
            var lowest = slideWindow.first().value;
            var biggest = slideWindow.last().value;
            if (biggest - lowest < result[1] - result[0]) {
                result[0] = lowest;
                result[1] = biggest;
            }
            var element = slideWindow.pollFirst();
