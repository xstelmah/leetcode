...
class Task1882 {
    ...
    void example1() {
        ...
        Assertions.assertArrayEquals(expected, assignTasks(servers, tasks));
    }
    ...
    void example2() {
        ...
        int[] expected = {26, 50, 47, 11, 56, 31, 18, 55, 32, 9, 4, 2, 23, 53, 43, 0, 44, 30, 6, 51, 29, 51, 15, 17, 22, 34, 38, 33, 42, 3, 25, 10, 49, 51, 7, 58, 16, 21, 19, 31, 19, 12, 41, 35, 45, 52, 13, 59, 47, 36, 1, 28, 48, 39, 24, 8, 46, 20, 5, 54, 27, 37, 14, 57, 40, 59, 8, 45, 4, 51, 47, 7, 58, 4, 31, 23, 54, 7, 9, 56, 2, 46, 56, 1, 17, 42, 11, 30, 12, 44, 14, 32, 7, 10, 23, 1, 29, 27, 6, 10, 33, 24, 19, 10, 35, 30, 35, 10, 17, 49, 50, 36, 29, 1, 48, 44, 7, 11, 24, 57, 42, 30, 10, 55, 3, 20, 38, 15, 7, 46, 32, 21, 40, 16, 59, 30, 53, 17, 18, 22, 51, 11, 53, 36, 57, 26, 5, 36, 56, 55, 31, 34, 57, 7, 52, 37, 31, 10, 0, 51, 41, 2, 32, 25, 0, 7, 49, 47, 13, 14, 24, 57, 28, 4, 45, 43, 39, 38, 8, 2, 44, 45, 29, 25, 25, 12, 54, 5, 44, 30, 27, 23, 26, 7, 33, 58, 41, 25, 52, 40, 58, 9, 52, 40};

        Assertions.assertArrayEquals(expected, assignTasks(servers, tasks));
    }

    public int[] assignTasks(int[] servers, int[] tasks) {
        var answers = new int[tasks.length];
        var loading = new int[servers.length]; // tests are passed, but in theory we can cross int limits 1 server x 10^5 requests x 10^5 (each request time)
        var free = new PriorityQueue<Integer>((i1, i2) -> servers[i1] - servers[i2] == 0 ? i1 - i2 : servers[i1] - servers[i2]);
        var busy = new PriorityQueue<Integer>((i1, i2) -> loading[i1] - loading[i2]);
        ...
        while (i < tasks.length) {
            while (!busy.isEmpty() && sec >= loading[busy.peek()]) free.add(busy.poll());
            if (free.isEmpty()) {
                sec = loading[busy.peek()];
                continue;
            }
            var freeServer = free.poll();
            loading[freeServer] = sec + tasks[i];
            busy.add(freeServer);
            answers[i] = freeServer;
            i++;
            if (sec < i) {
                sec++;
            }
